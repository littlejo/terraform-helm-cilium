locals {
  ...
  preflight_set_values = local.preflight_image != {} ? [{ name = "preflight.image.repository", value = "${local.preflight_image.url}/${local.preflight_image.image}" }, { name = "preflight.image.useDigest", value = "false" }] : []
